...
from transformers import LxmertConfig, PreTrainedModel, add_start_docstrings
from transformers.activations import ACT2FN, gelu
from transformers.file_utils import ModelOutput, add_start_docstrings_to_callable, add_code_sample_docstrings, \
    replace_return_docstrings
...
class LxmertEncoder(nn.Module):
    def __init__(self, config):
        ...
        #config_mod = copy(config)
        #config_mod.intermediate_size = config.hidden_size
        #self.l_output = LxmertLayer(config)
    ...
    def forward(
            ...
            activate_visual_feats: torch.Tensor = None
    ):
        ...
        if not activate_visual_feats:
            visual_feats = (torch.zeros(*visual_feats.shape[:2], self.r_layers[-1].output.dense.out_features).to(visual_feats.device))
            vision_hidden_states = vision_hidden_states + (visual_feats,)
            visual_encoder_outputs = (
                vision_hidden_states,
                None,
            )
            #lang_feats = self.l_output(lang_feats)[0]
            language_hidden_states = language_hidden_states + (lang_feats,)
...
class LxmertModelMod(LxmertPreTrainedModel):
    def __init__(self, config):
    ...
    def forward(
        ...
        encoder_outputs = self.encoder(
            ...
            output_attentions=output_attentions,
            activate_visual_feats=activate_visual_feats
        )
...
class LxmertForPreTraining(LxmertPreTrainedModel):
    def __init__(self, config):
        ...
        # Lxmert backbone
        self.lxmert = LxmertModelMod(config)
