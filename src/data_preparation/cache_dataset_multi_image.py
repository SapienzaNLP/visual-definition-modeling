...
def compute_lxmertembeddings_multi_images(examples, batch_size_lxmert, lxmert, crossmod_encoder_tokenizer):
    ...
    for idx, example in tqdm(examples.items(), desc='computing lxmert embeddings'):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--features_file", type=str, nargs='*',
                        ...
                        'data/in/babelpic.frcnn.all_imgs.other.0.npz',
                        'data/in/babelpic.frcnn.all_imgs.other.1.npz',
                        'data/in/babelpic.frcnn.all_imgs.other.2.npz',
                        'data/in/babelpic.frcnn.all_imgs.other.3.npz'])
    parser.add_argument("--dm_file", type=str,
                        default='/media/bianca/fluffy_potato/MultimodalGlosses/word+img.train.dm.txt')
    parser.add_argument("--babelpic_bns_file", type=str,
                        default='data/in/babelpic_bns.pkl')
    parser.add_argument("--out_dir", type=str,
                        default='data/in/training_multi_image_manmap/') #vl-models-bert-base_bart-decoder_train_multi_image/')
    ...
    parser.add_argument("--batch_size", type=int,
                        default=400)
    parser.add_argument("--cache_lxmert", action='store_true')
    parser.add_argument("--batch_size_lxmert", type=int,
                        default=4)
    parser.add_argument("--model_name_encoder", required=True)
    parser.add_argument("--model_name_decoder", required=True)
    ...
    pprint(vars(args))
    encoder_name = args.model_name_encoder #"bert-base-uncased"
    decoder_name = args.model_name_decoder
    ...
    decoder_tokenizer = AutoTokenizer.from_pretrained(decoder_name, fast=True)
    lxmert = None
    if cache_lxmert:
    ...
    examples_images = dict()
    kind2indices = dict()
    for k in kind2examples:
        if k == 'txt':
            continue
        kind_idxs = set([x for x in sorted(img2indices) for y in img2indices[x] if y[0] == k])
        print(len(kind2examples[kind]))
        for i in range(0, len(kind_idxs), batch_size):
            target_ids = sorted(img2indices)[i:i+batch_size]
            for f in features_file:
                ...
                for s, boxes, feats in tqdm(zip(data.get('synsets'), data.get('normalized_boxes'), data.get('features')), desc="Reading features file"):
                    img_id = s.split('_')[0]
                    if img_id not in img2indices or img_id not in target_ids:
                        continue
                    for kind, idx in img2indices[img_id]:
                        if kind != k:
                            continue
                        if idx in examples_images:
                            ex = examples_images[idx]
                        else:
                            ex = kind2examples[kind][idx].copy()
            ...
            if cache_lxmert:
                ...
                examples_images = compute_lxmertembeddings_multi_images(examples_images, batch_size_lxmert, lxmert, crossmod_encoder_tokenizer)
            with open(os.path.join(out_dir, kind, 'pkl_{}.pkl'.format(id_file)), 'wb') as writer:
                pkl.dump([examples_images[e] for e in examples_images], writer)  
            id_file += 1
